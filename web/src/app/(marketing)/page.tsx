      ...
      <section id="features" className="mx-auto max-w-6xl px-6 py-24">
        <div className="max-w-2xl">
          ...
          <p className="mt-4 text-muted">
            A focused feature set covering the full URL-shortener lifecycle —
            creation, distribution, analytics, and operations.
          </p>
        </div>
        <div className="mt-12 grid gap-px overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--border)] sm:grid-cols-2 lg:grid-cols-3">
          {features.map((f) => (
            <div
              key={f.title}
              className="bg-[var(--background)] p-6 transition hover:bg-[var(--surface)]"
            >
            ...
            {[
              {
                n: "01",
                t: "Submit a long URL",
                b: "POST /api/shorten with your API key. We validate, sanitize, and reserve a unique short code.",
              },
              {
                n: "02",
                t: "Redirect at the edge",
                b: "GET /:code reads from Redis first, falls back to Postgres, and 302s to the original URL.",
              },
              {
                n: "03",
                t: "Analyze clicks",
                b: "Each redirect logs a hashed click event for daily, referrer, and country aggregations.",
              },
            ].map((step) => (
              <li
                key={step.n}
      ...
      <section className="mx-auto max-w-6xl px-6 pb-24">
        <div className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-10 text-center">
          ...
          <p className="mx-auto mt-3 max-w-xl text-muted">
            Spin up an account, drop in a URL, and watch the analytics roll in.
          </p>
